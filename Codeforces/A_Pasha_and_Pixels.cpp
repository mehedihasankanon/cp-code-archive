...
bool f(vector<string> &v, ll a , ll b)
{
    bool lu = false, ld=false, ru=false, rd=false;
    // left up
    if(a-1>=0 && b-1>=0)
    {
        if(v[a-1][b-1] == '1' && v[a-1][b] == '1' && v[a][b-1] == '1') lu = true;
    }
    // left down
    if(a+1<=n-1 && b+1>=0)
    {
        if(v[a][b-1] == '1' && v[a+1][b-1] == '1' && v[a+1][b] == '1') ld = true;
    }
    // right up
    if(a-1>=0 && b+1<=m-1)
    {
        if(v[a-1][b] == '1' && v[a-1][b+1] == '1' && v[a][b+1] == '1') ru = true;        
    }
    // right down
    if(a+1<=n-1 && b-1<=m-1)
    {
        if(v[a][b+1] == '1' && v[a+1][b+1] == '1' && v[a+1][b] == '1') rd =true;        
    }
...
{   
    ...
    forsn(i,1,k+1)
    {
        ...
        //cout << boolalpha << f(v,a-1,b-1) << nl;
        if(f(v,a-1,b-1)) {out = i; break;}
    }
