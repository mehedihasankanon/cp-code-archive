...
void AmiEktaGadha()
{   
    int w,h,wst,q; cin >> h >> w >> wst >> q;
    map<pair<int,int>,string> stats;

    for(int i = 0; i < wst; i++) 
    {
        ll x, y; cin >> x >>y; stats[{x,y}] = "Waste";
    } 

    // 1 = grape, 3/0 = carrots, 2 = kiwis
    ...
    for(int i = 1; i <= h; i++)
    {
        for(int j = 1; j <= w; j++)
        {
            if(stats[{i,j}] != "Waste")  
            {
                if(n%3==0) stats[{i,j}] = "Grapes";
                else if(n%3==2) stats[{i,j}] = "Kiwis";
                else if(n%3==1) stats[{i,j}] = "Carrots";
                n++;
            }
        }
    }
