...
{   
    ...
    vector<ll> od, ev; ll maxodd=0,maxeven=0,stepev=0,stepod=0;
    for(auto u : mpe) {ev.push_back(u.second); maxeven = max(maxeven,u.second); stepev += u.second;}
    for(auto u : mpo) {od.push_back(u.second); maxodd = max(maxodd,u.second); stepod += u.second;}

    //dbg(maxeven); dbg(stepev); dbg(maxodd); dbg(stepod); 
    if(n%2==1) 
    {
        if(od.size()==1)
        {
    ...
    stepev -= maxeven; stepod -= maxodd;
    cout << stepev+stepod << nl;
    return;
