...
// status  : wa wa ac -- own implementation wrong, had to read editorial
...
{   
    ...
    {
        ll x, y, z; cin >> x >> y >> z;
        if(x+y+z==x || x+y+z==y || x+y+z==z) cout << "YES" << nl;
        else cout << "NO" << nl;
        return;
    }
    else forn(i,n) {cin >> v[i]; s.insert(v[i]);}
    sort(v.begin(),v.end());

    if(s.size()==2 || s.size() > 3) cout << "NO" << nl;
    else
    {
        if((s.size()==3 && *s.begin()+*s.rbegin()==0 && *(++s.rbegin())==0) || (s.size()==1 && *s.begin()==0)) cout << "YES" << nl;
        else cout << "NO" << nl;
    }
    return;*/

    vector<ll> v; ll n0 = 0, np = 0, nn = 0;
    // ami ekta gadha
    forn(i,n)
    {
        ll x; 
        cin >> x;
        if(x<0) {v.push_back(x); nn++;}
        else if(x>0) {v.push_back(x); np++;}
        else if(n0 == 0) {v.push_back(x); n0++;}  
    }

    if(nn > 2 || np > 2) cout << "NO" << nl;
    else
    {
        for(ll i = 0; i < v.size(); i++)
        {
            for(ll j = i+1; j < v.size(); j++)
            {
                for(ll k = j+1; k < v.size(); k++)
                {
                    ...
                    {
                        if(v[i]+v[j]+v[k] == v[l]) {fl = true;}
                    }
                    
                    if(!fl) {cout << "NO" << nl;return;}
                }
            }
        }
