...
void solve()
{   
    // a few missed cases here prolly
    /*ll n; cin >> n;
    vector<ll> v(n); map<ll,ll> m; unordered_set<ll> us1, us2;
    bool srt = true;
    ll st, en;
    ...
    else forn(i,n)
    {
        ...
        {
            if(st == -1 && m[v[i]]==1) st = i; 
            else if(m[v[i]]==1) en = i;
            else st = -1; 
        }
        else
        {
    ...
    if(st == -1) cout << us1.size() << nl;
    else 
    {
        forn(i,st) us2.insert(v[i]);
        cout << us2.size() << nl;
    }
