...
// link    : https://codeforces.com/contest/413/problem/B
// status  : tle, expected
...
{   
    ll n,m,k; cin >> n >> m >>k;
    // store in which chat which employee is
    vector<vector<int>> emp(m);
    forn(i,n)
    {
        forn(j,m)
        {
            ll x; cin >> x;
            if(x) emp[j].push_back(i+1);
        }
    }
    vector<ll> ntf(n+1);
