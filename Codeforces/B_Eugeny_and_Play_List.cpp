...
ll bs(ll q, vector<pair<ll,ll>> &v)
{
    ll l = 0, r = v.size(), mid;
    while(l < r-1)
    {
        mid = (l+r)/2;
        //dbg(mid);
        if(v[mid].first <= q && v[mid].second >= q) l = mid;
        else r = mid;
        dbg(l); dbg(r);
    }
...
{   
    ll n, m; cin >> n >> m;
    /*vector<short int> songs;
    forn(i,n)
    {
        ll cnt, time; cin >> cnt >> time; // dbg(cnt); dbg(time);
        forn(j,cnt*time) songs.push_back(i+1);
        //forn(i,sz(songs)) dbg(songs[i]);
        //dbg(songs.back());
    }
    forn(i,m)
    {
        ll v; cin >> v; cout << songs[v-1] << nl; 
    }*/

    ll t1 = 1, t2 = 1, cnt, time;
