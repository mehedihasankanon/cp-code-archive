...
// link    : https://vjudge.net/problem/HDU-1065
// status  : 
...
{   
    ld x, y; cin >> x >> y;
    ld area = ((3.1415926536*(x*x+y*y))/2.0); ll ans;

    /*ll l = 0, r = M, mid;
    forn(i,100)
    {
        mid = (l+r)/2;
        if(100.0*mid >= area) r = mid;
        else l = mid;
    }*/

    ans = area/50;

    cout << "Property " << i << ": This property will begin eroding in year " << ++ans << "." << nl;
    return;
}   
