...
// link    : https://vjudge.net/problem/CodeForces-1493B
// status  : wa
...
bool valid(ll x)
{
    if((x%10 == 0 ||x%10 == 1 ||x%10 == 2 ||x%10 == 5 ||x%10 == 8) && (x/10 == 0 ||x/10 == 1 ||x/10 == 2 ||x/10 == 5 ||x/10 == 8)) return true;
    else return false;
}
...
{   
    ...
    while(!(valid(h) and rev(h) < M and valid(m) and rev(m) < H))
    {
        if(m < M-1) m++;
        else {h++; m = 0;}
