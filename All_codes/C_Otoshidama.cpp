...
void AmiEktaGadha()
{   
    ll n, y; cin >> n >> y; y /= 1000; // a*10 + b*5 + c*1 = y hobe, and a+b+c=n hobe.
                                       // => 9*a+4*b=z=y-n hobe! --> LDE
                                       // (let z = y-n)
                                       // 
    ...
    else 
    {
        ...
        {
            ll z = y - n; 
            if(z%9==0 and n - (y-n)/9 >= 0) 
            {
                //a = (y-n)/9, b = 0, c = n-a
                cout << (y-n)/9 << " " << 0 << " " << n - (y-n)/9 << nl; return;
            }
            else if(z%4==0 and n - (y-n)/4 >= 0)
            {
                cout << 0 << " " << (y-n)/4 << " " << n - (y-n)/4 << nl; return;
            }
            else
            {
                ll a, b;
                for(ll a = z/9+1; a >= 0; a--)
                {
                    //dbg(a); dbg((z-9*a)/4); dbg(n - a - (z-9*a)/4);
                    if((z-9*a)%4==0 and z-9*a >=0 and n - a - (z-9*a)/4 >= 0) 
                    {
                        cout << a << " " << (z-9*a)/4 << " " << n - a - (z-9*a)/4 << nl;
                        return;
                    }
                }
                cout << "-1 -1 -1" << nl;
            ...
            /*ll z = y-n; // 9a + 4b = z => b = (z-9a)/4. also c >= 0
            ll a = z/9+1, b;

            for(;a >= 0; a--)
            {
                b = (z-9*a)/4;
                if((z-9*a)%4==0 and n-a-b>=0)
                {
                    cout << a << b << n-a-b << nl;
                    return;
                }
