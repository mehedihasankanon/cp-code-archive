...
// link    : https://codeforces.com/contest/2010/problem/C1
// status  : ac
...
{   
    ...
    if(s.size()%2==0)
    {
        ...
        {
            //string common = s.substr(len/2-i,len/2+i-1);
            string left = s.substr(0,len/2+i);
            string right = s.substr(len/2-i,len/2+i);
            //dbg(left); dbg(right);

            if(left == right) {cout << "YES" << nl << left << nl; return;} 
        }
    }
    else
    {
        ...
        {
            string left = s.substr(0,len/2+1+i);
            string right = s.substr(len/2-i,len/2+1+i);
            //dbg(left); dbg(right);
