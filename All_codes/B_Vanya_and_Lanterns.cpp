...
{   
    ll n, l; cin >> n >> l;
    vector<ll> lights; vector<ld> distance;
    forn(i,n)
    {
        ll x; cin >> x; lights.push_back(x);
    }
    sort(lights.begin(),lights.end());

    if(lights.front() != 0) distance.push_back(*lights.begin());
    if(lights.back() != l) distance.push_back(l-lights.back());

    forsn(i,1,n)
    {
        distance.push_back((lights[i]-lights[i-1])/2.0);
    }
    sort(distance.begin(),distance.end());

    cout << setprecision(15) << fixed << (ld)(distance.back()) << nl;
}   
