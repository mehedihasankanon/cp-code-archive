...
{   
    ...
    forsn(i,1,a%10+1) {a1[i]++;} //dbg(a1[i]);}
    forsn(i,1,b%10+1) {b1[i]++;} //dbg(b1[i]);}
    //forn(i,10) {dbg(i);dbg(a1[i]);dbg(b1[i]);}

    ll ans = a1[0]*(b1[5]+b1[0]);
    forsn(i,1,6)
    {
        ans += a1[i]*(b1[abs(5-i)]+b1[abs(10-i)]);
    }
    forsn(i,6,10) ans += a1[i]*(b1[15-i]+b1[10-i]);
