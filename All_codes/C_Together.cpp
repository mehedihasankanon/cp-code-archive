...
// link    : https://atcoder.jp/contests/abc072/tasks/arc082_a
// status  : re*4 ac //out of bounds errors// 10^5 == 100000, not 10000
...
{   
    ...
    vector<ll> count(sz,0),possible(sz,0); 
    forn(i,n) 
    {
        ll x; cin >> x; count[x]++;
    }

    forsn(i,1,sz) {possible[i] = count[i-1]+count[i]+count[i+1];}

    sort(possible.begin(),possible.end());

    cout << possible.back() << nl;
    return;
