...
// link    : https://codeforces.com/contest/39/problem/F
// status  : re ac ***because of wrong comp function.*** 
...
{   
    ll n, m, k; cin >> n >> m >> k;
    vector<ll> jum(m), mos(k); vector<pair<ll,ll>> des;
    ...
    {
        ll d = 0;
        forn(j,k) {if(mos[j]%jum[i]==0) d++;}
        //dbg(i+1); dbg(d);
        des.push_back({i+1,d}); //dbg(des.back().first);
    }

    sort(des.begin(),des.end(),comp); //dbg(des.size());
    //for(auto [u,v] : des) cout << u << " " << v << nl;
    //dbg(des[0].first);

    vector<ll> out;
    ll min = (des[0]).second, ans = 0, i = 0; //dbg(min);
    forn(i,m) 
    {
        if(des[i].second==min) out.push_back(des[i].first);
    }
