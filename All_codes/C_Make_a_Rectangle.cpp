...
// link    : https://atcoder.jp/contests/abc071/tasks/arc081_a
// status  : wa wa ac ***forgot to consider squares***
...
{   
    ...
    if(s.size()>=2 && m[*s.rbegin()]<4) cout << *s.rbegin() * *(++s.rbegin()) << nl;
    else if(s.size()>=2 && m[*s.rbegin()]>=4) cout << *s.rbegin() * *(s.rbegin()) << nl;
    else if(s.size()==1 && m[*s.rbegin()]>=4) cout << (*s.rbegin())*(*s.rbegin()) << nl;
    else cout << "0" << nl;
    return;
}   
