...
// link    : https://atcoder.jp/contests/abc366/tasks/abc366_b
// status  : ce ac

// ce: __max marsilam karone :P
...
{   
    ...
    forn(i,n) s[i].resize(sm,'*');

    //vector<string> out(sm);
    
    forn(i,sm)
    {
        string out;
        for(ll j = n-1; j >= 0; j--) out.push_back(s[j][i]);
        //dbg(out);
