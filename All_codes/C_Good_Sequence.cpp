...
// link    : https://atcoder.jp/contests/abc082/tasks/arc087_a
// status  : ac
...
{   
    ...
    sort(all(v),comp);
    ll current = *v.rbegin(), curcnt = 1, ans = 0;
    v.pop_back();
    if(v.empty())
    {
        if(current != curcnt) ans++;
        cout << ans << nl;
        return;
    }
    else while(!v.empty())
    {
        ...
        {
            if(curcnt < current) ans += curcnt;
            else if(curcnt > current) ans += (curcnt - current);
            current = *v.rbegin();
            curcnt = 1;
            v.pop_back();
        }
